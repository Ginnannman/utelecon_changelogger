# UTokyo Account本人確認サービスの連携解除について

| 言語 | 状態 | 公開ページ | ソース |
|---|---|---|---|
| 日本語 | 変更 | [`/utokyo_account/ident-myna/unlink/`](<https://utelecon.adm.u-tokyo.ac.jp/utokyo_account/ident-myna/unlink/>) | [`src/pages/utokyo_account/ident-myna/unlink/index.mdx`](<https://github.com/utelecon/utelecon.github.io/blob/18e2245e0d9b23fb32fa12781b0265e26a44d251/src/pages/utokyo_account/ident-myna/unlink/index.mdx>) |
| English | — | — | — |

## 日本語

```diff
  [UTokyo Account](/utokyo_account/) > [本人確認サービス](/utokyo_account/ident-myna/) > 連携解除
  # UTokyo Account本人確認サービスの連携解除について
+ [デジタル認証アプリの機能は，マイナアプリに統合されました](https://services.digital.go.jp/mynaapp/news/20260825-01/)．以下の説明では，「デジタル認証アプリ」は「マイナアプリ」と読み替えてください．
+ なお，マイナアプリは以下のリンクからダウンロードできます．
+ - [iOS](https://apps.apple.com/jp/app/%E3%83%9E%E3%82%A4%E3%83%8A%E3%82%A2%E3%83%97%E3%83%AA-%E6%97%A7%E3%83%9E%E3%82%A4%E3%83%8A%E3%83%9D%E3%83%BC%E3%82%BF%E3%83%AB%E3%82%A2%E3%83%97%E3%83%AA/id1476359069)
+ - [Android](https://play.google.com/store/apps/details?id=jp.go.cas.mpa&pcampaignid=web_share)
  ## はじめに
  このページでは，UTokyo Account本人確認サービスを既に利用している方が，「[事前連携](../link/)」を解除する手続きについて説明します．
```

[← 一覧へ](<../README.md>)
